...
export function AlertsPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title="Оповещения"
        description="Активные оповещения и каналы доставки для вашего торгового терминала."
      />

      <div className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
        <SectionCard title="Активные оповещения" description="Обзор последних сработавших торговых оповещений.">
        ...
        <SectionCard title="Каналы доставки" description="Куда могут быть направлены ваши оповещения.">
          <div className="space-y-3 rounded-3xl border border-slate-800/90 bg-slate-950/70 p-4">
            {['Email', 'Telegram', 'In-app'].map((channel) => (
              <div key={channel} className="flex items-center justify-between rounded-2xl border border-slate-800/90 bg-slate-900/80 px-4 py-3">
                ...
                <span className="text-xs uppercase tracking-[0.18em] text-slate-400">Включено</span>
              </div>
            ))}
          </div>
          <div className="rounded-3xl border border-slate-800/90 bg-slate-950/70 p-4">
            <div className="flex items-center gap-3 text-white">
              ...
              <span className="font-semibold">Надежность оповещения</span>
            </div>
            <p className="mt-3 text-sm text-slate-400">Оповещения доставляются с приоритетной маршрутизацией и низкой задержкой.</p>
          </div>
        </SectionCard>
      </div>
    </div>
